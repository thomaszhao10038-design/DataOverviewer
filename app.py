...
# --- Stage 1: Process CSV Files (Modified Code 1 Logic) ---
    ...
    for uploaded_file in uploaded_files:
        ...
        try:
            ...
            if df_full.shape[1] < max(col_indices) + 1:
                st.error(f"File **{filename}** has only {df_full.shape[1]} columns. Requested column index is out of bounds.")
                continue
            ...
            # 2. Date and Time formatting cleanup (ensure standard input for next stage)
...
# --- Stage 2 Helper: Process Single Sheet (Code 2 Logic) ---
def process_sheet(df, timestamp_col, psum_col):
    """
    Processes a single DataFrame sheet: combines Date/Time, cleans data, 
    ...
    df = df.rename(columns={psum_col: POWER_COL_OUT})
...
# --- Stage 2 Final: Build Excel (Code 2 Logic) ---
def build_output_excel(sheets_dict):
    ...
    for sheet_name, df in sheets_dict.items():
        ...
        for date in dates:
            ...
            ws.cell(row=merge_start, column=col_start, value="N/A").alignment = Alignment(horizontal="center", vertical="center")
...
def app():
    ...
    st.markdown("""
    This app performs a two-stage process:
    1. **Consolidation:** Upload multiple raw CSV files and specify the source columns.
    2. **Analysis:** The consolidated data is automatically processed, resampled to **10-minute intervals**, and filtered to keep only the active recording periods (excluding leading/trailing zeros).

    The final output is a highly formatted Excel file with daily data, charts, and a Total Load summary.
    """)
    st.write("---")

    # --- Sidebar: Column Configuration (Stage 1 Config) ---
    st.sidebar.header("⚙️ CSV Column Configuration (Stage 1)")
    st.sidebar.markdown("Enter the Excel column letters for Date, Time, and PSum. The data is read starting from **Row 3** (index 2).")
    ...
    if uploaded_files:
        if st.button(f"🚀 Run Full Pipeline on {len(uploaded_files)} File(s)"):
            st.info("Starting Stage 1: Consolidating Raw Data...")
            ...
            if not processed_raw_data_dict:
                st.error("Stage 1 failed: No files were successfully processed.")
                return
            ...
            for i, (sheet_name, df_raw) in enumerate(processed_raw_data_dict.items()):
                # Call process_sheet (which expects Date, Time, and PSum (W) columns from Stage 1)
                processed_df = process_sheet(df_raw, 'Date', PSUM_RAW_NAME)
